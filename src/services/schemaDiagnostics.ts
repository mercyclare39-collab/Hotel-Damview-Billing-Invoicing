...
import { propagationVarianceService } from './propagationVarianceService';
...
export interface LineItemParityComparison {
  index: number;
  ...
  propagatedParticulars: string;
  originalAmount: number;
  propagatedAmount: number;
  varianceNotes: string[];
}

export interface DocumentPropagationParityValidation {
  documentNumber: string;
  documentType: string;
  isIdentical: boolean;
  fieldsCheckedCount: number;
  differencesCount: number;
  lineItemsCountOriginal: number;
  lineItemsCountPropagated: number;
  lineItemsParity: {
    isIdentical: boolean;
    differences: string[];
    lineItemComparisons: LineItemParityComparison[];
  };
  detailsParity: {
    clientNameMatch: boolean;
    clientKraPinMatch: boolean;
    clientAddressMatch: boolean;
    clientPhoneMatch: boolean;
    clientEmailMatch: boolean;
    issueDateMatch: boolean;
    dueDateMatch: boolean;
    grossSubtotalMatch: boolean;
    discountMatch: boolean;
    subtotalMatch: boolean;
    vatAmountMatch: boolean;
    grandTotalMatch: boolean;
    amountPaidMatch: boolean;
    balanceDueMatch: boolean;
    statusMatch: boolean;
    variances: { field: string; originalVal: any; propagatedVal: any; severity: 'CRITICAL' | 'WARNING' }[];
  };
  summary: string;
}

/**
 * Rigorous Validation Check: Confirms if a document propagated from Google Sheets
 * differs from the original local record, especially for line items, particulars, and financial calculations.
 */
export function validatePropagatedDocumentParity(
  original: BillingDocument,
  propagated: BillingDocument,
  emitNotification: boolean = false
): DocumentPropagationParityValidation {
  const variances: { field: string; originalVal: any; propagatedVal: any; severity: 'CRITICAL' | 'WARNING' }[] = [];
  const lineItemDiffs: string[] = [];
  const lineItemComparisons: LineItemParityComparison[] = [];

  let fieldsChecked = 0;

  // 1. Client Details Parity
  const origName = String(original.clientName || '').trim();
  const propName = String(propagated.clientName || '').trim();
  const clientNameMatch = origName.toLowerCase() === propName.toLowerCase();
  fieldsChecked++;
  if (!clientNameMatch) {
    variances.push({ field: 'clientName', originalVal: origName, propagatedVal: propName, severity: 'CRITICAL' });
  }

  const origPin = normalizeKraPin(original.clientKraPin || '');
  const propPin = normalizeKraPin(propagated.clientKraPin || '');
  const clientKraPinMatch = !origPin || !propPin || origPin === propPin;
  fieldsChecked++;
  if (!clientKraPinMatch) {
    variances.push({ field: 'clientKraPin', originalVal: original.clientKraPin, propagatedVal: propagated.clientKraPin, severity: 'WARNING' });
  }

  const origPhone = normalizePhoneNumber(original.clientPhone || '');
  const propPhone = normalizePhoneNumber(propagated.clientPhone || '');
  const clientPhoneMatch = !origPhone || !propPhone || origPhone === propPhone;
  fieldsChecked++;
  if (!clientPhoneMatch) {
    variances.push({ field: 'clientPhone', originalVal: original.clientPhone, propagatedVal: propagated.clientPhone, severity: 'WARNING' });
  }

  const origEmail = String(original.clientEmail || '').trim().toLowerCase();
  const propEmail = String(propagated.clientEmail || '').trim().toLowerCase();
  const clientEmailMatch = !origEmail || !propEmail || origEmail === propEmail;
  fieldsChecked++;
  if (!clientEmailMatch) {
    variances.push({ field: 'clientEmail', originalVal: original.clientEmail, propagatedVal: propagated.clientEmail, severity: 'WARNING' });
  }

  const origAddr = String(original.clientAddress || '').trim();
  const propAddr = String(propagated.clientAddress || '').trim();
  const clientAddressMatch = !origAddr || !propAddr || origAddr === propAddr;
  fieldsChecked++;
  if (!clientAddressMatch) {
    variances.push({ field: 'clientAddress', originalVal: origAddr, propagatedVal: propAddr, severity: 'WARNING' });
  }

  // 2. Dates & Status Parity
  const issueDateMatch = String(original.issueDate || '').substring(0, 10) === String(propagated.issueDate || '').substring(0, 10);
  fieldsChecked++;
  if (!issueDateMatch) {
    variances.push({ field: 'issueDate', originalVal: original.issueDate, propagatedVal: propagated.issueDate, severity: 'WARNING' });
  }

  const dueDateMatch = String(original.dueDate || '').substring(0, 10) === String(propagated.dueDate || '').substring(0, 10);
  fieldsChecked++;
  if (!dueDateMatch) {
    variances.push({ field: 'dueDate', originalVal: original.dueDate, propagatedVal: propagated.dueDate, severity: 'WARNING' });
  }

  const statusMatch = String(original.status || '').toLowerCase() === String(propagated.status || '').toLowerCase();
  fieldsChecked++;
  if (!statusMatch) {
    variances.push({ field: 'status', originalVal: original.status, propagatedVal: propagated.status, severity: 'WARNING' });
  }

  // 3. Financial Totals Parity (float epsilon = 0.05 Ksh)
  const isClose = (a?: number | null, b?: number | null) => Math.abs((Number(a) || 0) - (Number(b) || 0)) <= 0.05;

  const grossSubtotalMatch = isClose(original.grossSubtotal, propagated.grossSubtotal);
  fieldsChecked++;
  if (!grossSubtotalMatch) {
    variances.push({ field: 'grossSubtotal', originalVal: original.grossSubtotal, propagatedVal: propagated.grossSubtotal, severity: 'CRITICAL' });
  }

  const discountMatch = isClose(original.discount, propagated.discount);
  fieldsChecked++;
  if (!discountMatch) {
    variances.push({ field: 'discount', originalVal: original.discount, propagatedVal: propagated.discount, severity: 'WARNING' });
  }

  const subtotalMatch = isClose(original.subtotal, propagated.subtotal);
  fieldsChecked++;
  if (!subtotalMatch) {
    variances.push({ field: 'subtotal', originalVal: original.subtotal, propagatedVal: propagated.subtotal, severity: 'CRITICAL' });
  }

  const vatAmountMatch = isClose(original.vatAmount, propagated.vatAmount);
  fieldsChecked++;
  if (!vatAmountMatch) {
    variances.push({ field: 'vatAmount', originalVal: original.vatAmount, propagatedVal: propagated.vatAmount, severity: 'CRITICAL' });
  }

  const grandTotalMatch = isClose(original.grandTotal, propagated.grandTotal);
  fieldsChecked++;
  if (!grandTotalMatch) {
    variances.push({ field: 'grandTotal', originalVal: original.grandTotal, propagatedVal: propagated.grandTotal, severity: 'CRITICAL' });
  }

  const amountPaidMatch = isClose(original.amountPaid, propagated.amountPaid);
  fieldsChecked++;
  if (!amountPaidMatch) {
    variances.push({ field: 'amountPaid', originalVal: original.amountPaid, propagatedVal: propagated.amountPaid, severity: 'CRITICAL' });
  }

  const balanceDueMatch = isClose(original.balanceDue, propagated.balanceDue);
  fieldsChecked++;
  if (!balanceDueMatch) {
    variances.push({ field: 'balanceDue', originalVal: original.balanceDue, propagatedVal: propagated.balanceDue, severity: 'CRITICAL' });
  }

  // 4. Deep Line Items Validation Check
  const origItems = original.lineItems || [];
  const propItems = propagated.lineItems || [];

  const maxItems = Math.max(origItems.length, propItems.length);
  if (origItems.length !== propItems.length) {
    lineItemDiffs.push(`Line items count mismatch: original has ${origItems.length} items, propagated has ${propItems.length} items.`);
  }

  for (let i = 0; i < maxItems; i++) {
    const oItem = origItems[i];
    const pItem = propItems[i];
    const varianceNotes: string[] = [];

    if (!oItem) {
      lineItemDiffs.push(`Line Item #${i + 1} added in propagated data: '${pItem.particulars}' (${pItem.amount} Ksh)`);
      lineItemComparisons.push({
        index: i + 1,
        particularsMatch: false,
        quantityMatch: false,
        daysMatch: false,
        rateMatch: false,
        discountMatch: false,
        amountMatch: false,
        originalParticulars: 'N/A (Missing)',
        propagatedParticulars: pItem.particulars || '',
        originalAmount: 0,
        propagatedAmount: pItem.amount || 0,
        varianceNotes: ['Item missing in original document'],
      });
      continue;
    }

    if (!pItem) {
      lineItemDiffs.push(`Line Item #${i + 1} missing in propagated data: '${oItem.particulars}' (${oItem.amount} Ksh)`);
      lineItemComparisons.push({
        index: i + 1,
        particularsMatch: false,
        quantityMatch: false,
        ...
        discountMatch: false,
        amountMatch: false,
        originalParticulars: oItem.particulars || '',
        propagatedParticulars: 'N/A (Missing)',
        originalAmount: oItem.amount || 0,
        propagatedAmount: 0,
        varianceNotes: ['Item missing in propagated document'],
      });
      continue;
    }

    const oPart = String(oItem.particulars || '').trim();
    const pPart = String(pItem.particulars || '').trim();
    const particularsMatch = oPart.toLowerCase() === pPart.toLowerCase();
    if (!particularsMatch) {
      varianceNotes.push(`Particulars: '${oPart}' vs '${pPart}'`);
      lineItemDiffs.push(`Item #${i + 1} particulars changed from '${oPart}' to '${pPart}'`);
    }

    const quantityMatch = (Number(oItem.quantity) || 1) === (Number(pItem.quantity) || 1);
    if (!quantityMatch) {
      varianceNotes.push(`Quantity: ${oItem.quantity} vs ${pItem.quantity}`);
      lineItemDiffs.push(`Item #${i + 1} quantity changed from ${oItem.quantity} to ${pItem.quantity}`);
    }

    const daysMatch = (Number(oItem.days) || 1) === (Number(pItem.days) || 1);
    if (!daysMatch) {
      varianceNotes.push(`Days: ${oItem.days} vs ${pItem.days}`);
      lineItemDiffs.push(`Item #${i + 1} duration changed from ${oItem.days} to ${pItem.days}`);
    }

    const rateMatch = isClose(oItem.rate, pItem.rate);
    if (!rateMatch) {
      varianceNotes.push(`Rate: Ksh ${oItem.rate} vs Ksh ${pItem.rate}`);
      lineItemDiffs.push(`Item #${i + 1} unit rate changed from Ksh ${oItem.rate} to Ksh ${pItem.rate}`);
    }

    const discountItemMatch = isClose(oItem.discount || 0, pItem.discount || 0);
    if (!discountItemMatch) {
      varianceNotes.push(`Discount: Ksh ${oItem.discount || 0} vs Ksh ${pItem.discount || 0}`);
      lineItemDiffs.push(`Item #${i + 1} discount changed from Ksh ${oItem.discount} to Ksh ${pItem.discount}`);
    }

    const amountMatch = isClose(oItem.amount, pItem.amount);
    if (!amountMatch) {
      varianceNotes.push(`Amount: Ksh ${oItem.amount} vs Ksh ${pItem.amount}`);
      lineItemDiffs.push(`Item #${i + 1} total amount changed from Ksh ${oItem.amount} to Ksh ${pItem.amount}`);
    }
    ...
    lineItemComparisons.push({
      ...
      discountMatch,
      amountMatch,
      originalParticulars: oPart,
      propagatedParticulars: pPart,
      originalAmount: oItem.amount || 0,
      propagatedAmount: pItem.amount || 0,
      varianceNotes,
    });
  ...
  const lineItemsIdentical = lineItemDiffs.length === 0;
  const isIdentical = variances.length === 0 && lineItemsIdentical;
  const differencesCount = variances.length + lineItemDiffs.length;

  let summary = isIdentical
    ? `Document ${original.documentNumber} is 100% identical between local ERP and propagated Google Sheets data (${fieldsChecked} fields and ${origItems.length} line items matched).`
    : `Document ${original.documentNumber} has ${differencesCount} variance(s): ${[...variances.map((v) => v.field), ...lineItemDiffs.slice(0, 2)].join(', ')}.`;

  if (isIdentical) {
    propagationVarianceService.resolveVariance(original.documentNumber);
  } else {
    const hasMissingLineItems = lineItemDiffs.some(
      (d) => d.toLowerCase().includes('missing') || d.toLowerCase().includes('count mismatch')
    );
    const varianceType = hasMissingLineItems
      ? 'MISSING_LINE_ITEMS'
      : variances.some((v) => v.field.includes('Total') || v.field.includes('balance') || v.field.includes('vat'))
      ? 'TOTALS_MISMATCH'
      : 'DATA_MISMATCH';

    propagationVarianceService.registerVariance({
      ...
      summary,
      differencesCount,
      missingLineItemsCount: Math.abs(origItems.length - propItems.length),
      lineItemDiffs,
      fieldDiffs: variances.map((v) => v.field),
      resolved: false,
    });

    // Auto change and resolve documents when Propagation Variance Detected to accurate data
    if (propagationVarianceService.isAutoResolveEnabled()) {
      setTimeout(() => {
        propagationVarianceService.executeAutoResolve(original, propagated).catch((err) => {
          console.warn('[PropagationVariance] Auto-resolve deferred:', err);
        });
      }, 50);
    }
  }
  ...
  if (emitNotification) {
    appNotificationService.notifyPropagationValidation(
      original.documentNumber,
      isIdentical,
      summary,
      { variances, lineItemDiffs }
    );
  }
  ...
  return {
    ...
    documentType: original.documentType,
    isIdentical,
    fieldsCheckedCount: fieldsChecked + maxItems * 6,
    differencesCount,
    lineItemsCountOriginal: origItems.length,
    lineItemsCountPropagated: propItems.length,
    lineItemsParity: {
      isIdentical: lineItemsIdentical,
      differences: lineItemDiffs,
      lineItemComparisons,
    },
    detailsParity: {
      clientNameMatch,
